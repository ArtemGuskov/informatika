...
int main() {
    ...
    map<string, Coord> nodes;
    vector<EdgeItem> edges;
    std::tie(nodes, edges) = read_graphml("london_road_network.graphml");
    // print_edges(edges);
    ...
    string start_street_query = "Penryn Street";      // Название улицы для старта
    string end_street_query = "Clark Street";      // Название улицы для финиша
    ...
    if (start_index == -1 || end_index == -1) {
        ...
        if (path.empty()) {
            ...
            std::cout << "Время работы алгоритма: " << elapsed_ms << " мс" << std::endl;
